...
export const userSettings = sqliteTable("user_settings", {
  ...
  userId: text("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  darkMode: integer("dark_mode", { mode: "boolean" }).default(sql`0`), // SQLite doesn't have boolean
  soundEnabled: integer("sound_enabled", { mode: "boolean" }).default(sql`1`),
