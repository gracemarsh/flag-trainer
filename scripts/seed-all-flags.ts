...
export const countriesData: [string, string, string, number, string, number][] =
  [
    // North America
...
async function main() {
  ...
  console.log(
    `🌎 Preparing to seed ${countriesData.length} countries/territories...`,
  );
  ...
  try {
    ...
    const existingFlagsQuery = await client.execute(
    ...
    for (const [
      ...
      try {
        ...
        if (
          ...
          console.log(
            `✅ Added ${successCount}/${countriesData.length} flags... (Last: ${name})`,
          );
        }
      } catch (error) {
        errorCount++;
    ...
    const finalCountQuery = await client.execute(
      "SELECT COUNT(*) as count FROM flags",
    );
    ...
    if (finalCount === countriesData.length) {
      ...
      console.warn(
        `⚠️ ${finalCount} out of ${countriesData.length} flags were inserted.`,
      );
